...
class GJMAA_Service_Woocommerce {
	...
	public function addProduct( $allegroProduct, $newMethod = false ) {
		if ( $newMethod ) {
			...
			if ($this->getProfile()->getData('profile_affiliate_products') && class_exists('GJWA_PRO')) {
				update_post_meta( $post_id, '_product_url', 'https://allegro.pl/oferta/'.($allegroProduct['external_id'] ?? $allegroProduct['id']) ); // URL produktu zewnętrznego
				update_post_meta( $post_id, '_button_text', 'Kup teraz' );
				wp_set_object_terms( $post_id, 'external', 'product_type' );
			} else {
				wp_remove_object_terms( $post_id, 'external', 'product_type' );
			}
			...
			foreach ( $this->_wooCommerceMeta as $meta_key => $meta_value ) {
				...
				switch ( $meta_key ) :
					...
					case '_price':
					case '_regular_price':
						if ( $this->getProfile()->getData('profile_sync_price') || $post['new'] ) {
						...
						break;
					case '_stock':
						$meta_value = $this->getProfile()->getData('profile_sync_stock') || $post['new'] ? $allegroProduct[ 'stock' ] : $originalValue;
						break;
					case '_stock_status':
						$meta_value = $this->getProfile()->getData('profile_sync_stock') || $post['new'] ? ( $allegroProduct[ 'stock' ] > 0 ? 'instock' : 'outofstock' ) : $originalValue;
						break;
