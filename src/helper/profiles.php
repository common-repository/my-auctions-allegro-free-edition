...
class GJMAA_Helper_Profiles
{
    ...
    public function getFieldsData($type = 'form')
    {
		...
		if (GJMAA::getService('woocommerce')->isEnabled()) {
			$form += [
				'profile_affiliate_products' => [
					'type' => 'select',
					'name' => 'profile_affiliate_products',
					'label' => "Twórz produkty zewnętrzne",
					'source' => 'yesno',
					'disabled' => !class_exists('GJWA_PRO'),
					'help' => "Opcja wymaga aktywnej wtyczki <a href='https://wphocus.com/produkt/woocommerce-allegro-pro-integracja/'>Woocommerce Allegro</a>"
				]
			];
        }
